...
> **HuggingFace token:** No longer needed at build time. Each user enters their own
> HF token directly in the app the first time they download the AI model. The token
> is stored in Android Keystore / iOS Keychain via `flutter_secure_storage`.
